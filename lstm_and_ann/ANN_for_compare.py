...
from attention_utils import get_activations, get_data_recurrent, get_data_recurrent_ANN, get_data_recurrent_1
...
def model_DNN():
    ...
    inputs_nn_features = Input(shape=(NN_FEATURES,))  # 默认用全局变量设置为5列nn特征输入
    print("shape of inputs_nn_features：", inputs_nn_features.shape)
    nn_dense_1 = Dense(512, activation='relu')(inputs_nn_features)  # relu  640
    print("shape of nn_dense_1: ", nn_dense_1.shape)
    # nn_dense_2 = Dense(100, activation='relu')(nn_dense_1)  # 100
    # nn_dense_3 = Dense(100, activation='relu')(nn_dense_2)  # 100

    nn_flatten = MyFlatten()(nn_dense_1)
...
def model_ANN():
    ...
    inputs_nn_features = Input(shape=(NN_FEATURES,))  # 默认用全局变量设置为5列nn特征输入
    print("shape of inputs_nn_features：", inputs_nn_features.shape)
    nn_dense_1 = Dense(20, activation='relu')(inputs_nn_features)  # relu  640
    print("shape of nn_dense_1: ", nn_dense_1.shape)
...
def model_attention_applied_after_lstm():
    ...
    nn_out_final = Dense(600, activation='relu')(nn_dense_1)  # ······  (600 relu)
    con = merge([attention_mul_next, nn_out_final], name='last_merge', mode='concat')
    before_output_1 = Dense(30, activation='relu')(con)  # (30 relu) 可以做进一步降低尝试（小幅度）(20-80)
...
if __name__ == '__main__':
    ...
    inputs_2, outputs, primary_data, scaler = get_data_recurrent_ANN(N, TIME_STEPS, INPUT_DIM, NN_FEATURES)
    ...
    # 100 - 512 - 0.05
    # 用LSTM的版本
    # m.fit([inputs_1[:-last_for_train], inputs_2[:-last_for_train]], outputs[:-last_for_train], epochs=80, batch_size=512, validation_split=0.1)
    # y_predictions = m.predict([inputs_1[-last_for_train:], inputs_2[-last_for_train:]])

    # 用ANN的版本
    m.fit(inputs_2[:-last_for_train], outputs[:-last_for_train], epochs=1, batch_size=512, validation_split=0.1)
    y_predictions = m.predict(inputs_2[-last_for_train:])
    ...
    for (train, test) in floder.split(X, y):
        ...
        history = m.fit(inputs_2[:-last_for_train], outputs[:-last_for_train], epochs=128, batch_size=512, validation_split=0.1, callbacks=[reduce_lr])
        y_predictions = m.predict(inputs_2[-last_for_train:])
        ...
        print(np.array(outputs[-last_for_train:]).shape)
        print(np.array(primary_data[-last_for_train:]).shape)
        y_real = concatenate((np.array(outputs[-last_for_train:]).reshape(len(outputs[-last_for_train:]), 1),
                              primary_data[-last_for_train:]), axis=1)
        y_real = scaler.inverse_transform(y_real)
        y_real = y_real[:, 0]
